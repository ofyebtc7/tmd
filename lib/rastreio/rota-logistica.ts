...
/**
 * Monta a sequência de eventos de rastreio de um envio Plugmax.
 *
 * Rota: Praia Grande/SP (CTCE-PG, origem fixa) → CIDADE HUB → cliente.
 * CIDADE HUB = capital do estado do cliente (proxy do ponto mais próximo).
 * Quando o hub é a própria origem (ex.: Praia Grande/SP), a rota é direta,
 ...
 * separada. Eventos com data_evento futura ficam ocultos da página, pois as
 * RPCs de rastreio filtram por `data_evento <= now()`.
 */
export function montarEventosRota({
  ...
  const origem = cidades.find((c) => c.nome_local === 'CTCE-PG')
  if (!origem) return []
