...
function resolverStatus(
  ...
  // Fallback quando o eventos informa a transição (mesmo se status vier vazio
  // ou com literal desconhecido do gateway).
...
async function findPagamentoByTransactionId(
  ...
  if (error || !data) {
    console.error('pagamento_nao_encontrado', {
...
async function findPagamentoPorClienteReference(
  ...
  if (error || !data) {
    console.log('pagamento_nao_encontrado_por_client_reference', {
...
async function gerarRotaLogistica(
  ...
  // Valida CEP: deve ter exatamente 8 dígitos numéricos antes de chamar ViaCEP
  ...
  // ─── VALIDAÇÃO DEFENSIVA ───────────────────────────────────────────────────
  // Bug 4 (sessão anterior): não inserir evento duplicado (rastreamento_id,
  // titulo) nem datas fora do cronograma (anterior ao último registro / futuro
  // anormal além do último evento previsto).
  ...
  const eventosPendentes = eventosInsert.filter((ev) => {
    ...
    if (ultimaDataEventoMs > 0 && dataMs < ultimaDataEventoMs) {
      console.error('evento_rota_data_anterior_ao_ultimo_ignorado', {
        rastreamento_id: rastreamentoId,
        titulo: ev.titulo,
        data_evento: ev.data_evento,
        ultima_data_evento: new Date(ultimaDataEventoMs).toISOString(),
      })
      return false
    }

    if (dataMs > limiteFuturoMs) {
      console.error('evento_rota_data_futura_anormal_ignorado', {
        rastreamento_id: rastreamentoId,
        titulo: ev.titulo,
        data_evento: ev.data_evento,
        limite_futuro: new Date(limiteFuturoMs).toISOString(),
      })
      return false
    }
...
async function processarEvento(
  ...
  if (!gatewayPaymentId) {
    console.log('pagamento_sem_id_correlacionar', { status: statusGateway })
    return true
  }
  ...
  console.log('pagamento_correlacionado', {
    pagamento_id: pagamento.id,
    pedido_id: pagamento.pedido_id,
  })

  if (pagamento.status === 'pago') {
    console.log('pagamento_ja_processado', {
      pagamento_id: pagamento.id,
      evento: normalizado.evento,
    })
    return true
  ...
  try {
    ...
    console.log('pagamento_atualizado', {
      pagamento_id: pagamento.id,
      novo_status: novoStatusPagamento,
    })
    ...
    if (novoStatusPagamento === 'pago') {
      // ── Purchase via Meta Conversions API (CAPI) — server-side ──
      // Disparado SOMENTE aqui, quando o pagamento é confirmado como "pago"
      // (resp.md Passo 1). A guarda de status 'pago' acima evita re-disparo
      // em webhooks duplicados. Falha de analytics não quebra o fluxo.
      try {
        const { data: pedidoPago, error: erroPedidoPago } = await supabaseAdmin
          .from('pedidos')
        ...
        if (erroPedidoPago) {
          console.error('erro_buscar_pedido_para_capi', erroPedidoPago)
        }
      ...
      if (!rastreamentoExistente) {
        ...
        try {
          const { data: pedidoComCliente, error: erroBuscarCliente } = await supabaseAdmin
            .from('pedidos')
            .select('clientes ( cep, cidade, estado )')
            .eq('id', pagamento.pedido_id)
            .single()

          if (erroBuscarCliente) {
            console.error('erro_buscar_cliente_para_rota', erroBuscarCliente)
          }
        ...
        console.log('rastreamento_ja_existente', {
          pedido_id: pagamento.pedido_id,
        })
      }
    }
...
export async function POST(request: NextRequest) {
  ...
  if (assinatura) {
    if (!validarAssinatura(rawBody, assinatura)) {
      console.error('webhook_assinatura_invalida', {
    ...
    // Sem assinatura NÃO processa: impede que qualquer pessoa marque
    // pedido/pagamento como "pago" falsamente (fraude de pagamento).
    console.error('webhook_sem_assinatura_rejeitado', {
      headers: headersRelevantes,
    })
